...
impl PacketRx {
    /// Opens a capture socket bound to one interface, filtered to `what`.
    ///
    /// A port set for the fake-TCP carrier, which rotates between them while it
    /// runs: the filter covers the whole set from the start, so rotation needs
    /// neither a new socket nor a new filter — which matters because the thread
    /// reading this one is blocked in `recv` and cannot be handed either. A bare
    /// protocol number for a carrier that has no ports.
    ///
    ...
    pub fn open(interface: &str, what: bpf::Match<'_>) -> io::Result<Self> {
...
mod tests {
    ...
    fn a_missing_interface_fails_before_the_socket_is_opened() {
        ...
        assert!(PacketRx::open("definitely-not-real", bpf::Match::Ports(&[9999])).is_err());
    }
    ...
    fn a_socket_can_be_opened_on_loopback() {
        let rx = PacketRx::open("lo", bpf::Match::Ports(&[9999])).expect("open");
